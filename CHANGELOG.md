...
- LLM API 配置新增独立 QPS、并发线程数、重试次数与重试间隔；留空继承全局默认，切换配置后新任务使用对应参数。
- 新增片段请求重试设置：支持 OpenAI SDK 引擎，次数 0–100，固定间隔 0–300 秒；默认 -1 保留引擎策略。
- 自定义重试关闭 SDK 内层重试，避免次数叠加；仅重试超时、连接异常、408/409/429 与 5xx，等待期间支持取消。
- 配置检查与任务提交共用参数选择规则，保留旧配置兼容性及显式零值。

- 新增 Docker 镜像发布 CI，接入统一发版脚本；从稳定版 tag 构建并验证服务健康和 OCR，支持手动补发。

## v5.3.1 兼容性更新 - 2026-09-11

- 将 Zotero 兼容上限从 `9.*` 扩展到 `10.0.*`；通过固定 `release/update.json` 向已安装的 v5.3.1 分发兼容性更新，不更换原 XPI 或服务端版本。
- 修正原更新元数据与 v5.3.1 XPI 不一致的 SHA-512，避免自动更新包校验失败。
- Zotero 10.0.2 隔离资料库验证：插件启用、翻译菜单、任务窗口、设置页与 LLM 编辑器初始化通过。未执行真实 LLM 翻译。

## v5.3.1 - 2026-08-21

- 修复服务端将 `pdf2zh_next` 翻译和 PDF 选项传入错误配置层级的问题。
- 修复 Homebrew 后台服务在 `/` 工作目录下写入翻译结果失败的问题，并增加可写工作目录兜底。
- 增加 runtime settings 回归测试，确认输出目录、语言、QPS、输出模式和 OCR 等选项实际生效。
